...
export function App() {
  ...
  return <main><h1>Document library</h1><form onSubmit={upload}><input name="file" type="file" accept=".pdf,.docx,.txt,.md,.markdown" /><button>Upload</button></form>{error && <p role="alert">{error}</p>}<ul>{documents.map((document) => <li key={document.id}><strong>{document.title}</strong> — {document.original_filename} ({document.status}) {document.error_message && <span role="alert">: {document.error_message}</span>} {document.status === "failed" && <button onClick={() => void action(document.id, "/retry", "POST")}>Retry</button>} <button onClick={() => void action(document.id, "/reindex", "POST")}>Re-index</button> <button onClick={() => void action(document.id, "", "DELETE")}>Delete</button></li>)}</ul></main>;
}
